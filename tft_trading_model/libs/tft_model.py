...
def apply_gating_layer(x,
    ...
    if use_time_distributed:
        ...
        activation_layer = tf.keras.layers.Dense(hidden_layer_size,
                                                 activation=activation)(x)
        gated_layer = tf.keras.layers.Dense(hidden_layer_size,
...
class TemporalFusionTransformer(object):
    ...
    def __init__(self, raw_params, use_cudnn=False):
        ...
        self.embedding_downsample_hidden_size = int(
            params['embedding_downsample_hidden_size'])
        self.embedding_downsample_size = int(
            params['embedding_downsample_size'])

        print(
            "Debug input_size={}, crypto_input_size={}, embedding_input_size={}"
            .format(self.input_size, self.crypto_input_size,
    ...
    def get_tft_embeddings(self, all_inputs):
        ...
        if all_inputs.get_shape().as_list(
        )[-1] != self.crypto_input_size + self.embedding_downsample_size:
            raise ValueError(
                'Illegal number of inputs! Inputs observed={}, expected={}'.
                format(all_inputs.get_shape().as_list()[-1],
                       self.crypto_input_size +
                       self.embedding_downsample_size))
        ...
        for i in range(num_categorical_variables):
            ...
            embedding = tf.keras.Sequential([
                ...
                tf.keras.layers.Embedding(self.category_counts[i],
                                          embedding_sizes[i],
        ...
        for i in range(regular_inputs.shape[-1]):
            if i not in self._known_regular_input_idx \
                ...
                e = convert_real_to_embedding(regular_inputs[Ellipsis,
                                                             i:i + 1])
                unknown_inputs.append(e)

        if unknown_inputs + wired_embeddings:
            unknown_inputs = tf.keras.backend.stack(unknown_inputs +
                                                    wired_embeddings,
                                                    axis=-1)
        else:
            unknown_inputs = None
        ...
        known_combined_layer = tf.keras.backend.stack(known_regular_inputs +
                                                      known_categorical_inputs,
                                                      axis=-1)
    ...
    def training_data_cached(self):
        return TFTDataCache.contains('train') and TFTDataCache.contains(
            'valid')

    def cache_batched_data(self, data, cache_key, num_samples=-1):
        if num_samples > 0:
            TFTDataCache.update(
                self._batch_sampled_data(data, max_samples=num_samples),
                cache_key)
        else:
    ...
    def _batch_sampled_data(self, data, max_samples):
        if max_samples < 1:
            raise ValueError(
                'Illegal number of samples specified! samples={}'.format(
                    max_samples))
        ...
        sampled_data = {
            ...
            'active_entries':
            np.ones_like(outputs[:, self.num_encoder_steps:, :]),
            'time': time,
    ...
    def _build_base_graph(self):
        ...
        # Inputs.
        all_inputs = tf.keras.layers.Input(shape=(
            time_steps,
            combined_input_size,
        ))
        ...
        embedding_hidden = tf.keras.layers.TimeDistributed(
            tf.keras.layers.Dense(
                self.embedding_downsample_hidden_size,
        ...
        inputs = concat([
            crypto_inputs[:, :, :num_regular_variables],
        ...
        def static_combine_and_mask(embedding):
            ...
            for i in range(num_static):
                e = gated_residual_network(embedding[:, i:i + 1, :],
                                           self.hidden_layer_size,
                                           dropout_rate=self.dropout_rate,
                                           use_time_distributed=False)
        ...
        def lstm_combine_and_mask(embedding):
            # Add temporal features
            _, time_steps, embedding_dim, num_inputs = embedding.get_shape(
            ).as_list()
        ...
        def get_lstm(return_state):
            if self.use_cudnn:
                ...
                lstm = tf.keras.layers.LSTM(self.hidden_layer_size,
                                            return_sequences=True,
                                            return_state=return_state,
                                            stateful=False,
                                            activation='tanh',
        ...
        input_embeddings = concat([historical_features, future_features],
                                  axis=1)

        lstm_layer, _ = apply_gating_layer(lstm_layer,
                                           self.hidden_layer_size,
                                           self.dropout_rate,
                                           activation=None)
        ...
        expanded_static_context = K.expand_dims(static_context_enrichment,
                                                axis=1)
        ...
        decoder, _ = apply_gating_layer(decoder,
                                        self.hidden_layer_size,
                                        activation=None)
    ...
    def build_model(self):
        with tf.variable_scope(self.name):
            ...
            adam = tf.keras.optimizers.Adam(lr=self.learning_rate,
                                            clipnorm=self.max_gradient_norm)
    ...
    def fit(self, train_df=None, valid_df=None):
        ...
        if os.path.exists(tmp_checkpont):
            ...
            print('Cannot load from {}, skipping ...'.format(
                self._temp_folder))
    ...
    def evaluate(self, data=None, eval_metric='loss'):
        ...
        metric_values = self.model.evaluate(x=inputs,
                                            y=np.concatenate(
    ...
    def predict(self, df, return_targets=False):
        ...
        combined = self.model.predict(inputs,
                                      workers=16,
                                      use_multiprocessing=True,
                                      batch_size=self.minibatch_size)
