...
class GenericDataFormatter(abc.ABC):
    ...
    def get_column_definition(self):
        ...
        def _check_single_column(input_type):
            ...
            length = len(
                [tup for tup in column_definition if tup[2] == input_type])
        ...
        identifier = [
            tup for tup in column_definition if tup[2] == InputTypes.ID
        ]
        time = [tup for tup in column_definition if tup[2] == InputTypes.TIME]
        real_inputs = [
            tup for tup in column_definition
            if tup[1] == DataTypes.REAL_VALUED and tup[2] not in
    ...
    def _get_input_columns(self):
        return [
            tup[0] for tup in self.get_column_definition()
            if tup[2] not in {InputTypes.ID, InputTypes.TIME}
        ]

    def _get_crypto_columns(self):
        return [
            tup[0] for tup in self.get_column_definition() if tup[2] not in
            {InputTypes.ID, InputTypes.TIME, InputTypes.EMBEDDING}
        ]
    ...
    def _get_tft_input_indices(self):
        ...
        def _extract_tuples_from_data_type(data_type, defn):
            return [
                tup for tup in defn if tup[1] == data_type
                and tup[2] not in {InputTypes.ID, InputTypes.TIME}
            ]
        ...
        locations = {
            ...
            'embedding_downsample_hidden_size':
            256,
            'embedding_downsample_size':
            4,
            'output_size':
